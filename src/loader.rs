...
fn dicom_element_smallvec_f64(dicom_element: &DataElement<InMemDicomObject<StandardDataDictionary>>) -> &smallvec::SmallVec<[f64; 2]>{
    let primitive = dicom_element.value().primitive().unwrap();
    let float_array = match primitive {
...
pub fn load_oct(path: String) -> Result<(ArrayBase<OwnedRepr<u8>, Dim<IxDynImpl>>, String, Vec<f64>), Error> {
    ...
    let pixel_volume = Array::from_shape_vec(shape, pixel_data.to_vec()).unwrap();
